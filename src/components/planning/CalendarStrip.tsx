...
export default function CalendarStrip() {
  ...
  if (!polygon) {
    return (
      <div className="h-28 bg-retro-panel border-t border-retro-green-muted flex items-center justify-center text-sm text-retro-green-dim">
  ...
  return (
    <div className="bg-retro-panel border-t border-retro-green-muted">
      <div className="flex items-center gap-3 px-3 py-2 border-b border-retro-green-muted">
        <span className="text-xs font-bold text-retro-green uppercase tracking-wider shrink-0">14-Day Forecast</span>
        <div className="flex items-center gap-2 ml-auto">
          <span className="text-xs text-retro-green-dim shrink-0">Clear pass &lt;</span>
          <input
            type="range"
            min={0}
            max={100}
            ...
            className="w-24"
          />
          <span className="text-xs font-bold text-retro-green w-8 text-right">{cloudThreshold}%</span>
        </div>
      </div>
      <div className="flex overflow-x-auto gap-2 px-2 py-2">
        {weatherLoading && (
          <div className="flex items-center justify-center w-full py-4 text-sm text-retro-green-dim">
            Loading weather...
          </div>
        )}
        {weather?.map((day) => (
